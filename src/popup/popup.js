...
document.addEventListener("DOMContentLoaded", function () {
  ...
  reminderInput.addEventListener("focus", function () {
  ...
  reminderInput.addEventListener("input", function () {
    ...
    charCounter.classList.remove("warning", "near-limit");
...
submitForm.addEventListener("submit", (e) => {
  ...
  if (reminder.length > 250 || reminder.length < 3) {
    handleErrorMessage(
      false,
      "Reminders must be between 3 - 250 characters in length"
    );
    return;
  } else {
...
function displayReminders(url) {
  chrome.storage.sync.get(url, function (result) {
    ...
    // Show empty state if no reminders exist
    if (reminders.length === 0) {
      const emptyState = document.createElement("div");
      emptyState.className = "empty-state";
      emptyState.innerHTML = `
                <div class="empty-state-icon">🦜</div>
                <p class="empty-state-title">No reminders yet!</p>
                <p class="empty-state-subtitle">Add your first reminder to get started.</p>
            `;
      remindersListContainer.appendChild(emptyState);
      return;
    }
...
function editReminder(url, reminderToEdit) {
  if (reminderToEdit) {
    const reminderEle = document.querySelector(
      `li[data-id="${reminderToEdit}"]`
    );

    // Store original text to restore on ESC
    const textNode = reminderEle.firstChild;
    const originalText = textNode.textContent;
    reminderEle.dataset.originalText = originalText;

    // Remove any existing handlers to prevent duplicates
    if (reminderEle.keypressHandler) {
      reminderEle.removeEventListener("keypress", reminderEle.keypressHandler);
    }
    if (reminderEle.keydownHandler) {
      reminderEle.removeEventListener("keydown", reminderEle.keydownHandler);
    }

    reminderEle.contentEditable = "true";
    reminderEle.classList.add("editing");
    ...
    iconContainer.className = "icon-save";
    iconContainer.onclick = () => saveReminder(url, reminderToEdit);

    // Create and store the keypress handler for Enter key
    ...
    // create and store the keydown handler for ESC key
    reminderEle.keydownHandler = function (event) {
      if (event.key === "Escape") {
        event.preventDefault();
    ...
    reminderEle.addEventListener("keydown", reminderEle.keydownHandler);
  ...
  if (reminderToSave) {
    ...
    if (updatedText.length > 250 || updatedText.length < 3) {
      ...
      errorMessage.textContent =
        "Reminders must be between 3 - 250 characters.";
      ...
      if (reminderEle.keypressHandler) {
        reminderEle.removeEventListener(
          "keypress",
          reminderEle.keypressHandler
        );
        delete reminderEle.keypressHandler;
      }
      if (reminderEle.keydownHandler) {
        reminderEle.removeEventListener("keydown", reminderEle.keydownHandler);
        delete reminderEle.keydownHandler;
      }
...
function exitEditMode() {
  ...
  if (editingReminder) {
    ...
    if (editingReminder.keypressHandler) {
      editingReminder.removeEventListener(
        "keypress",
        editingReminder.keypressHandler
      );
      delete editingReminder.keypressHandler;
    }
    if (editingReminder.keydownHandler) {
      editingReminder.removeEventListener(
        "keydown",
        editingReminder.keydownHandler
      );
      delete editingReminder.keydownHandler;
    }
